...
<div class="w3-container">
	...
	$contasql= mysqli_query($conexao,"SELCET profissionais.id_inscricao, profissionais.nome, profissionais.crefito as crefitonumero, profissionais.votou, profissionais.forma, 
	profissionais.id_usuario, profissionais.data_horario FROM profissionais, profissionais_enderecos, profissionais_enderecos.uf
	WHERE crefito.profissionais=crefito.profissionais_enderecos AND profissionais.votou='sim' ORDER BY profissionais.data_horario");
			...
			while($dadosp=mysqli_fetch_array($qprocessos)){
				...
				if($x%2==0) { $cor="#FFFFFF"; }
							...
							$texto2=$texto2.'<tr>
						      ...
						      <td width="66%" height="1">'.$dadosp["nome"].' - '.$dadosp["crefitonumero"].'</td>
							...
							<tr>
						      ...
						      <td width="66%" height="1" style="background:<?=$cor?>; border:0; padding:0"><?=utf8_encode($dadosp["nome"])." - ".$dadosp["crefitonumero"]?></td>
							  ...
							  if($dadosp["forma"]!=NULL) { 
								$e_nome=$dadosp["nome"];
								$e_crefito=$dadosp["crefitonumero"];
					 ...
					 $pagenamecsv = "relatorio_votacao_computado.csv";
                      $pagename = "relatorio_votacao.html";
						...
						if($fw == strlen($texto)) {
								...
								$gerarcsv=fwrite($fp,utf8_encode($e_carrega));
								fclose($fp);
