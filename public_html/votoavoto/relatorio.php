...
<div class="w3-container">
	...
	$contasql= mysqli_query($conexao,"select id_inscricao,nome,crefito,votou,forma,id_usuario,data_horario from profissionais WHERE votou='sim'");
	...
	from profissionais WHERE votou='sim'
	 order by data_horario desc limit $inicio, $registros";

	?>
	<tr>
	<td width="1%" height="1" style="background:<?=$cor?>; border:0; padding:0">
	</td>
		<td colspan="2" width="66%" height="1" style="background:<?=$cor?>; border:0; padding:0"><b><font color="#000000"><b><?=$total?></b> votos por correspondência computados até o momento</font></b>
	<br><b>Páginas: </b>  
<?php
for($i=1; $i < $numPaginas + 1; $i++){
	?>
	<input type="hidden" name="txtnome2<?=$i?>" id="txtnome2<?=$i?>" 
	value="relatorio.php?pagina=<?=$i?>"/>
	<input type="button"  value="<?=$i?>" onclick="getDados(2<?=$i?>);" style="border: #000 solid 0px; background-color:transparent; font-size:10px"  class="w3-bar-item w3-button w3-green" />&nbsp;
	<?php 
							  ...
							  if($dadosp["forma"]!=NULL) { 
							    $datetime = $dadosp["data_horario"];
									$orgdate = date("d/m/Y H:i", strtotime($datetime ));
								 	echo "Voto ".$dadosp["forma"]."  <i> 
								 	Registrado por [".$sqlusr["usuario"]."] em ".$orgdate."
									 </i>"; 
